...
- 因为现在的react网站，都是只有首次加载时，才加载整页，路由切换时，也不会触发整页加载，一次性写到全局解构中，也不会有性能问题（毕竟，[IconFont](https://www.iconfont.cn/) 脚本的动态创建也是这样的流程）。
...
| 参数             | 说明                                                                 | 类型                                                 | 默认值 | 必须 |
| ---------------- | -------------------------------------------------------------------- | ---------------------------------------------------- | ------ | ---- |
| type             | 填充类型                                                             | `fill`\|`line`                                       | `fill` | 否   |
| name             | 图标名称                                                             | `string`                                             | -      | 是   |
| viewBox          | 视图框，不会作用于 `svg`，仅当子元素 `symbol` 中不存在，自动使用此值 | `string`                                             | -      | 是   |
| path             | `Path` 标签属性                                                      | `TExtendedSVGPathProps` \| `TExtendedSVGPathProps[]` | -      | 是   |
