...
describe('symbol 组件', () => {
  ...
  it('应正确渲染 Symbol，无 prefix', () => {
    ...
    expect(svg.querySelector('symbol:first-child')?.id === 'ks-icon-earth').toBeTruthy();
  });

  it('应正确渲染 Symbol，有 prefix，classname', () => {
    const icons = [
      {
        viewBox: '0 0 1024 1024',
        icons: [
    ...
    const { container } = render(
      <IconProvider icongroup={icons} prefix='test'>
        <div>test</div>
      </IconProvider>
    );
    ...
    const symbol = svg.querySelector('symbol:first-child') as SVGSVGElement;
    expect(symbol.id === 'ks-icon-test-earth').toBeTruthy();

    expect(symbol.classList.contains('ABC')).toBeTruthy();

    const path = symbol.querySelector('path:first-child') as SVGPathElement;
    expect(path.classList.contains('abc')).toBeTruthy();
  });
});
